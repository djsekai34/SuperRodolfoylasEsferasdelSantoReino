...
public class Cell : MonoBehaviour
{
    ...
    {
        if (Bola_Cell != null)
            Instantiate(Bola_Cell, puntoDeEmision.position, transform.rotation);
    }
    ...
    IEnumerator EfectoEstatua()
    {
        ...
        // Ajustamos un pequeño margen para que este no atraviese el suelo
        float margenF3 = 0.3f;
        // Calculamos la posicion exacta del destino que se hara de la siguiente forma: Posición Actual - Distancia + Margen
        Vector3 posSueloFinal = posOriginalHijo - new Vector3(0, distanciaAlSueloFinal - margenF3, 0);
        ...
        string nombreAnimMorir = "CellMuere";
        ...
        while (tiempo < duracionTotal)
        {
            ...
            // Bloqueamos la animacion hasta el 70% en su posicion original 
            if (progreso < 0.70f)
            {
            ...
            {
                // A partir del 30% restante iniciamos la bajada hacia abajo
                float pBajada = (progreso - 0.70f) / 0.30f;
                // Usamos un SmoothStep para que la caida sea chill
                float pSuave = Mathf.SmoothStep(0, 1, pBajada);
                // Movemos el sprite del enemigo hacia el sitio que deseamos
                sr.transform.localPosition = Vector3.Lerp(posOriginalHijo, posSueloFinal, pSuave);
            }
