...
public class Hit : MonoBehaviour
{
    ...
    {
        if (Bola_Hit != null)
            Instantiate(Bola_Hit, puntoDeEmision.position, transform.rotation);
    }
    ...
    IEnumerator EfectoEstatua()
    {
        ...
        // Guardamos la posición original (donde se queda flotando al principio)
        Vector3 posOriginalHijo = sr.transform.localPosition;
        // Calculamos el suelo real para el último frame
        Vector3 posFinalHijo = posOriginalHijo - new Vector3(0, distanciaAlSueloFinal, 0);
        ...
        // Esperamos un poco para que veamos bien el cuerpo en el aire
        yield return new WaitForSeconds(0.4f);

        // Lo mandamos al suelo de golpe
